...
def main():
    print("Creating Conda environment 'ccc_env'...")
    ...
    env_python = os.path.join(base_path, "envs", "ccc_env", "bin", "python")
    if not os.path.exists(env_python):
        # Try looking in local envs dir if configured
        env_python = os.path.join(os.getcwd(), "envs", "ccc_env", "bin", "python")
    ...
    if not os.path.exists(env_python):
        print("Could not locate python executable for ccc_env. Please activate manualy and run 'pip install -e .'")
        return
    ...
    run_command(f"'{env_python}' -m ipykernel install --user --name=ccc_env --display-name 'Python (ccc_env)'")

    print("\nSetup complete!")
    print("To use: conda activate ccc_env")
